...
def codegen(func):
    ...
    def decorate(fields):
        ...
        repl_co_names = (
            *co_names[:(start := co_names.index('_0'))], *fields, *co_names[start+len(fields):])
        repl_co_varnames = (*co_varnames[:(start := co_varnames.index('_0'))], *fields,
                            *co_varnames[start+len(fields):]) if '_0' in co_varnames else co_varnames
